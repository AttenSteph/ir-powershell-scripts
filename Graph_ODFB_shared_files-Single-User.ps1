...
# Great script from https://github.com/michevnew/PowerShell/blob/master/Graph_ODFB_shared_files.ps1 originally under MIT License and retaining MIT license in this version.
# improved documentation in this comment section and added a single user hack
...
# with application permissions in graph and not delegation permissions, grant the following
# User.Read.All
# Sites.ReadWrite.All
# Directory.Read.All
# grant admin consent to these permissions in the app registration
# create a client secret and copy out the VALUE
# In the variables to set are below lower in the script set these:
# $tenantID
# $appID
# $client_secret
# if you are running against a single user enter their upn, which is usually their primary email in the line below that looks like this. Replace someuser@contoso.com
# $GraphUsers = $GraphUsers.Where({ $_.userPrincipalName -eq "someuser@contoso.com"})
# Otherwise the script runs against the entire tenants user base.
# If running a single user consider the command line options "-ExpandFolders -depth 999" to enumerate all files in the OneDrive, otherwise it runs a very low depth scan.
...
$tenantID = "<guid>" #your tenantID GUID found in the "Overview" section of Entra
$appID = "<guid>" #the GUID of your app taken from the main page of the application registration
$client_secret = "<longString>" #client secret VALUE for the app
...
# Steph - single user "fix"
$GraphUsers = $GraphUsers.Where({ $_.userPrincipalName -eq "userupnusuallyemail@contoso.com"})
